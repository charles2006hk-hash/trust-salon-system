    ...
    <div className="bg-[#080808] min-h-screen text-gray-200 p-8 font-sans selection:bg-[#D4AF37] selection:text-black">
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          ...
          {/* 模組 1：POS 收銀 (Admin, Manager, Reception 可見) */}
          ...
          {/* 模組 4：財務報表 (Admin, Manager, Staff 可見) */}
          {['admin', 'manager', 'staff'].includes(userData.role) && (
            <div onClick={() => router.push('/admin/finance')} className="bg-gradient-to-br from-[#1a1a1a] to-[#080808] p-8 rounded-[32px] border border-[#D4AF37]/30 hover:border-[#D4AF37] cursor-pointer transition-all group shadow-[0_0_30px_rgba(212,175,55,0.1)] relative overflow-hidden flex flex-col justify-between h-full">
              ...
              <div>
                <div className="w-16 h-16 rounded-2xl bg-[#D4AF37] flex items-center justify-center mb-6 group-hover:scale-110 transition-transform shadow-lg relative z-10">
                  <i className="fa-solid fa-file-invoice-dollar text-2xl text-black"></i>
                </div>
                <h3 className="text-lg font-bold text-white mb-2 relative z-10">
                  {userData.role === 'staff' ? '我的薪資單' : '財務與業績分析'}
                </h3>
                <p className="text-xs text-gray-400 leading-relaxed relative z-10">
                  {userData.role === 'staff' 
                    ? '查看當月個人服務客數、創造業績與實得抽成明細。' 
                    : '查看全店營業額、未消耗總負債，與髮型師抽成排行報表。'}
                </p>
              </div>
            </div>
          )}
